...
<div style="display:flex;align-items:center;gap:.75rem;margin-bottom:1.25rem;">
    ...
    <p class="breadcrumb" style="margin:0;"><a href="/courses"><?= __('courses.subjects') ?></a> › <?= htmlspecialchars($subject['name'], ENT_QUOTES) ?></p>
</div>
<div class="page-hd">
    <h1>
    ...
    <div class="hd-right">
        ...
        <a href="/courses/export?subject_id=<?= (int)$subject['id'] ?>" class="btn btn-secondary"><?= $iconExp ?> <?= __('courses.export') ?></a>
        <a href="/themes/create?subject_id=<?= (int)$subject['id'] ?>" class="btn btn-primary"><?= __('courses.new_theme') ?></a>
    </div>
</div>

<?php if (empty($themes)): ?>
    <p class="empty"><?= __('courses.no_themes') ?></p>
<?php else: ?>
    <div id="items">
        <?php foreach ($themes as $t): ?>
            <div class="card" onclick="location.href='/chapters?theme_id=<?= (int)$t['id'] ?>'">
                ...
                <div class="card-actions" onclick="event.stopPropagation()">
                    <a href="/courses/export?theme_id=<?= (int)$t['id'] ?>" class="btn-icon btn-download" title="Export"><?= $iconExp ?></a>
                    ...
                    <form method="post" action="/themes/delete">
                        ...
                        <button type="button" class="btn-icon btn-delete" title="Delete"
                                onclick="esConfirm('<?= htmlspecialchars(sprintf(__('courses.delete_confirm'), addslashes($t['name'])), ENT_QUOTES) ?>',()=>this.closest('form').submit())"><?= $iconDel ?></button>
                    </form>
                </div>
            </div>
        <?php endforeach; ?>
