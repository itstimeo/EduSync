...
<div style="display:flex;align-items:center;gap:.75rem;max-width:500px;margin:0 auto 1.25rem;">
    ...
    <p class="breadcrumb" style="margin:0;max-width:none;">
        ...
        <?= __('courses.upload_title') ?>
    </p>
</div>

<div class="form-card">
    <h1><?= __('courses.upload_title') ?></h1>
    <form method="post" action="/documents/upload?chapter_id=<?= (int)$chapter['id'] ?>" enctype="multipart/form-data" novalidate>
        ...
        <div class="field">
            ...
            <span class="field-err" id="err-file"><?= __('courses.file_required') ?></span>
            <p class="field-hint"><?= __('courses.file_hint') ?></p>
        </div>
        <p class="req-note"><span class="req">*</span> <?= __('common.required') ?></p>
        <div class="form-actions">
            <button type="submit" class="btn btn-primary"><?= __('courses.upload') ?></button>
        </div>
    </form>
</div>
