...
<?php else: ?>
    <div id="items">
        <?php foreach ($subjects as $s): ?>
            <div class="card" onclick="location.href='/themes?subject_id=<?= (int)$s['id'] ?>'">
                <div class="card-strip" style="background:<?= htmlspecialchars($s['color'], ENT_QUOTES) ?>"></div>
                <div class="card-body">
                    <div class="card-name"><?= htmlspecialchars($s['name'], ENT_QUOTES) ?></div>
                </div>
                <div class="card-actions" onclick="event.stopPropagation()">
                    <a href="/courses/export?subject_id=<?= (int)$s['id'] ?>" class="btn-icon btn-download" title="Export"><?= $iconExp ?></a>
                    <a href="/courses/edit?id=<?= (int)$s['id'] ?>" class="btn-icon btn-edit" title="Edit"><?= $iconEdit ?></a>
...
<script>
(function(){
    var key='es_view_subjects', v=localStorage.getItem(key)||'list';
    apply(v);
